...
export default class AuthenticationPrompt extends Scene {
  ...
  finalise_fingerprint_if_available() {
    ...
    return fp
      ...
      .catch(console.log.bind(console, 'fp error'));
  }

  reset_fingerprint() {
    this.finalise_fingerprint_if_available()
      .then(this.init_fingerprint_if_available.bind(this))
      .then((_) => {
  ...
  success_action_and_navigate() {
    this.props.route
      .auth_success_action()
      .then((_) => {
        this.navigator.resetTo(this.props.route.auth_success_destination);
      })
      ...
      .then(function(item) {
        if (!item) {
          ...
          return Promise.reject(
            'no password found - user probably not registered'
          );
        }
  ...
  render_authorisation_prompt_message_and_feedback() {
    const smallText = this.state.fingerprint
      ? 'Authenticate either with your PIN or fingerprint'
    ...
    return (
      <View style={{ marginTop: 20, marginBottom: 35 }}>
        <View style={style.largeTextRow}>
          <Text
            style={Object.assign({}, style.registrationFont, style.largeFont)}
          >
            Please authenticate
          </Text>
        </View>
        <View style={[style.smallTextRow]}>
          <Text
            style={Object.assign({}, style.registrationFont, style.smallFont)}
          >
            {smallText}
          </Text>
        </View>
        ...
        >
          {this.state.show_feedback && (
            <Text style={{ fontWeight: 'bold' }}>{this.state.feedback}</Text>
          )}
        </View>
      </View>
    );
